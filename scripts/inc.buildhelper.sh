  ...
  if getapksforapi "$package" "$usearch" "$usemaxapi" "$useminapi"; then
    ...
    for dpivariant in $(echo "$sourceapks" | tr ' ' ''); do #we replace the spaces with a special char to survive the for-loop
      ...
      if [ -z "$baseversionname" ]; then
        ...
        printf "%6s-%-2s %-61s %34s" "$usearch" "$api" "$1" "$baseversionname"  # use $1 instead of $package to show the foldername packagename instead of the getapkproperties-name to get the setupwizard name right
        if [ -n "$logfile" ]; then
          printf "%6s-%-2s| %-61s| %-34s|" "$usearch" "$api" "$1" "$baseversionname" >> "$logfile"
        fi
      fi
